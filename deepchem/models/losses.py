...
class VAE_KLDivergence(Loss):
  ...
  This loss represents KL divergence losses between normal distribution
  (using parameter of distribution) based on  _[1].
  ...
  Sample: batch_size = 2, hidden_space = 2
  >>> import numpy as np
  >>> import torch
  >>> import tensorflow as tf
  >>> logvar = np.array([[1.0,1.3],[0.6,1.2]])
...
class ShannonEntropy(Loss):
  ...
  Sample: batch_size = 2, num_of_variable = 2
  >>> import numpy as np
  >>> import torch
  >>> import tensorflow as tf
  ...
  References
  ----------
  .. [1] Chen, Ricky Xiaofeng. "A Brief Introduction to Shannon’s Information Theory."
     arXiv preprint arXiv:1612.09316 (2016).
  """
